...
public class UISpriteManager : MonoBehaviour
{
	...
	public Texture uiElementsTexture;
	public Texture uiElementsTextureHD;
	public TextAsset uiElementsConfig;
	public TextAsset uiElementsConfigHD;
	...
	public void loadTextureAndPrepareForUse()
	{
		...
		if( texture == null ) {
			...
			if ( UI.Instance.isHD ) {
				texture = uiElementsTextureHD;
                 uiElementsTexture = null;
			} else {
				texture = uiElementsTexture;
                 uiElementsTextureHD = null;
			}


			if (texture == null ) {
				Debug.LogError( "UI texture not set!" );
			}
		}
	...
	private Dictionary<string, UITextureInfo> loadTexturesFromTexturePackerJSON(string filename)
	{
		...
		if( asset == null ){ 
			//Debug.Log( "Could not find Texture Packer json config file in Resources folder: " + filename );

			if ( UI.Instance.isHD ) {
				//Debug.Log( "Attempting to load HD assets from Editor values ");
				asset = uiElementsConfigHD;
			} else {
				//Debug.Log( "Attempting to load assets from Editor values ");
				asset = uiElementsConfig;	
			}

			if (asset == null ) {
				Debug.LogError( "UI config not set!" );
			}
		}
